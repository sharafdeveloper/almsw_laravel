    ...
    <div class="max-w-7xl mx-auto" x-data="{ showDelete: false, deletingId: null, deletingLabel: '' }">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-6 gap-3">
            ...
            <div class="flex items-center gap-2">
                ...
                <a href="{{ route('sale-invoice.create') }}" class="inline-flex items-center px-4 py-2 bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-lg shadow whitespace-nowrap">
                ...
                <table class="min-w-full divide-y divide-gray-200 dark:divide-[#222b38]">
                    <thead class="bg-gray-50 dark:bg-[#0f1220]">
                        <tr>
                            ...
                            <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Cash Received</th>
                    ...
                    <tbody class="bg-white dark:bg-[#0b0e14] divide-y divide-gray-100 dark:divide-[#11151c]">
                        @forelse($invoices as $inv)
                            ...
                            <td class="px-4 py-3 text-sm whitespace-nowrap">{{ optional($inv->customer)->name }}</td>
                            <td class="px-4 py-3 text-sm whitespace-nowrap">{{ $inv->bill_date->toDateString() }}</td>
                            <td class="px-4 py-3 text-sm text-center">
                                {{-- Product: wrap rehne do --}}
                                {{ $inv->items->pluck('product.name')->filter()->implode(', ') ?: '-' }}
                            </td>
                            <td class="px-4 py-3 text-sm text-right whitespace-nowrap">
                                {{ $inv->items->sum('weight') > 0 ? number_format($inv->sub_total / $inv->items->sum('weight'), 2) : '-' }}
                            </td>
                            <td class="px-4 py-3 text-sm text-right whitespace-nowrap">
