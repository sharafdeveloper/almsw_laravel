    ...
    <div x-data="dashboard({{ json_encode($stats) }}, '{{ $from }}', '{{ $to }}')" class="max-w-7xl mx-auto">
        ...
        {{-- @if(auth()->user()->isAdmin())
            <div class="flex flex-wrap gap-3">
                <a href="{{ route('cashbook') }}" class="inline-flex items-center px-5 py-3 bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-lg shadow hover:opacity-95">
                    <i class="fa-solid fa-book mr-2"></i> View Cashbook
                </a>
                <a href="{{ route('balance-sheet') }}" class="inline-flex items-center px-5 py-3 bg-gradient-to-r from-emerald-600 to-teal-600 text-white rounded-lg shadow hover:opacity-95">
                    <i class="fa-solid fa-scale-balanced mr-2"></i> View Balance Sheet
                </a>
            </div>
        @endif --}}
    </div>
