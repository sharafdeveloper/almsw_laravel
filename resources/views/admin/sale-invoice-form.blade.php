...
@section('content')
            ...
            <div class="bg-white dark:bg-[#0b0e14] border border-gray-200 dark:border-[#1f2937] rounded-lg p-4 shadow-sm mb-4">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div x-data="customerSearch({
                        initId: {{ $invoice?->customer_id ?? 'null' }},
                        initName: '{{ $invoice?->customer ? $invoice->customer->name . ' (' . $invoice->customer->city . ')' : '' }}'
                    })">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-2 text-sm">
                        ...
                        <div class="flex justify-between items-center pt-2">
                            <span>Discount</span>
                            <input type="text" inputmode="decimal" pattern="^\d*(\.\d+)?$" x-model="discount" name="discount" placeholder="0" class="w-32 px-2 py-1 border rounded text-right bg-white dark:bg-[#081118] border-gray-200 dark:border-[#1a202c]">
                        </div>
                        <div class="flex justify-between items-center pt-2">
    ...
    <script>
        function customerSearch({ initId, initName }) {
    return {
        search: initName || '',
        selectedId: initId || '',
        results: [],
    ...
    <script>
        function saleInvoiceForm(config) {
            return {
                ...
                discount: config.invoiceId ? config.discount : '',
