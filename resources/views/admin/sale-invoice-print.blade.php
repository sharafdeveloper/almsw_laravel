...
<head>
    <meta charset="UTF-8">
    <title>Bill {{ $invoice->formattedId() }}</title>
    @php
        $co = config('pos.company');
        $itemsTotal = (float) $invoice->sub_total;                 // sum of line items
        $labour     = (float) $invoice->labour_cost;
        $loading    = (float) $invoice->loading;
        $subTotal   = $itemsTotal + $labour + $loading;            // image's "Sub Total"
        $cash       = (float) $invoice->cash_received;
        $discount   = (float) $invoice->discount;
          
        $netTotal   = $subTotal - $discount - $cash;                      // image's "Net Total"
    @endphp
    <style>
...
<table style="width: 100%; border-collapse: collapse; margin: 4px 0 8px 0; font-size: 14px; font-weight: bold; font-family: 'Times New Roman', Georgia, serif;">
    ...
    <table class="meta">
        ...
        <tr>
            <td>Customer Name : <u style="font-size: 15px;">{{ optional($invoice->customer)->name }}</u></td>
            <td class="right"></td>
        </tr>
    </table>

    <div class="memo-bar">Bill / Cash Memo</div>
        ...
        <tbody>
            @foreach($invoice->items as $i => $it)
                <tr>
                    ...
                    <td class="c">{{ rtrim(rtrim(number_format((float)$it->rate, 2), '0'), '.') }}</td>
            ...
            <!-- NAYA -->
                <tr>
                    ...
                    <td class="c" style="border:none; border-bottom:1px solid #000; font-weight:bold;">
                        {{ rtrim(rtrim(number_format($invoice->items->sum('weight') > 0 ? $itemsTotal / $invoice->items->sum('weight') : 0, 2), '0'), '.') }}
                    </td>
                    <td class="r" style="border:none; border-bottom:1px solid #000; font-weight:bold;">{{ number_format($itemsTotal, 0) }}</td>
                </tr>
        </tbody>
    </table>

    <table class="totals">
        <tr><td class="label">Labour Cost:</td><td class="val">{{ number_format($labour, 0) }}</td></tr>
        <tr><td class="label">Loading Cost:</td><td class="val">{{ number_format($loading, 0) }}</td></tr>
        <tr><td class="label">Sub Total:</td><td class="val">{{ number_format($subTotal, 0) }}</td></tr>
        <tr><td class="label">Discount:</td><td class="val">{{ number_format($discount, 0) }}</td></tr>
        <tr><td class="label">Cash Receive :</td><td class="val">{{ number_format($cash, 0) }}</td></tr>
