...
{
    public const METHODS = ['Cash', 'Bank Transfer', 'Cheque', 'Online Transfer'];
    ...
    public function index(Request $request)
    {
        ...
        $query = Payment::with('customer')->orderBy('payment_date', 'asc')->orderBy('id', 'asc');
    ...
    public function store(Request $request)
    {
        ...
        DB::transaction(function () use ($in, $out, &$createdAny) {
            if (!empty($in['customer_id']) && !empty($in['amount']) && (float) $in['amount'] > 0) {
                $p = Payment::create([
            ...
            if (!empty($out['customer_id']) && !empty($out['amount']) && (float) $out['amount'] > 0) {
                $p = Payment::create([
