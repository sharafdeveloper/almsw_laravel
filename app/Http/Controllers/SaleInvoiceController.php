...
class SaleInvoiceController extends Controller
{
    ...
    // {
    //     $invoices = SaleInvoice::with('customer', 'items.product')
    //         ->orderBy('id', 'asc')
    //         ->paginate(15);
    ...
    public function index(Request $request)
    {
        $q = $request->input('q', '');
        $invoices = SaleInvoice::with('customer', 'items.product')
            ->when($q, fn($query) => $query->whereHas('customer', fn($query) => $query->where('name', 'like', "%{$q}%")))
            ->orderBy('id', 'desc')
            ->paginate(15)
            ->withQueryString();
        return view('admin.sale-invoice', compact('invoices', 'q'));
    }
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route('sale-invoice', $invoice)->with('success', 'Sale invoice created.');
    }
    ...
    public function update(Request $request, SaleInvoice $sale_invoice)
    {
        ...
        return redirect()->route('sale-invoice', $invoice)->with('success', 'Sale invoice updated.');
    }
    ...
    private function persistInvoice(?SaleInvoice $existing, array $data): SaleInvoice
    {
        ...
        $total = round($subTotal + $labour + $loading - $discount, 2);
